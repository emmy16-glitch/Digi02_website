...
const applications: readonly Application[] = [
  {
    category: 'Operational application / Energy',
    title: 'Turn field evidence into maintenance decisions.',
    summary: 'For energy teams that need inspection findings, asset context and operational priorities to reach the same decision-makers without delay.',
    href: '/solutions/skygrid',
    label: 'Explore SkyGrid',
    image: gridVisual,
    imageAlt: 'Black African field engineer taking a distance measurement in protective equipment',
    imagePosition: 'center 37%',
  },
  {
    category: 'Operational application / Mobility',
    title: 'Keep movement visible from dispatch to arrival.',
    summary: 'For mobility and logistics teams that need vehicle activity, route progress and operating status to remain connected across the day.',
    href: '/solutions/digivolt',
    label: 'Explore DigiVolt',
    image: fleetVisual,
    imageAlt: 'Black African truck driver at work in a transport vehicle',
    imagePosition: 'center 50%',
  },
  {
    category: 'Operational application / Enterprise',
    title: 'Give every team the same operating picture.',
    summary: 'For organisations bringing payroll, workflow, records and reporting into one dependable enterprise environment.',
    href: '/solutions/enterprise-systems',
    label: 'Explore Enterprise Systems',
    image: enterpriseVisual,
    imageAlt: 'Black African business and technology team working together around a laptop and notes',
    imagePosition: 'center 47%',
  },
]
  ...
  return (
    <main className="work-reference-page">
      <section className="work-reference-hero" aria-labelledby="work-reference-title" style={{ backgroundImage: `url(${workHeroVisual})` }}>
        <Container className="work-reference-hero__inner">
          <p className="work-reference-kicker">Our Work</p>
          <h1 id="work-reference-title">Technology proven where the <span>work is real.</span></h1>
          <p>Digi02 partners with organisations that need clearer operations—not more disconnected tools. These selected engagements show how field intelligence, payments, public-service workflows and enterprise systems can be engineered around the way work actually moves.</p>
          <a className="work-reference-hero__cta" href="#featured-work">Explore selected work <span aria-hidden="true">↓</span></a>
        </Container>
      </section>

      <section className="work-reference-featured" id="featured-work" aria-labelledby="featured-case-title">
        <Container>
          <header className="work-reference-section-heading"><p>Featured case study</p><h2 id="featured-case-title">See the field. Understand the operation.</h2></header>
          <article className="work-reference-featured__card">
            <div className="work-reference-featured__media"><img src={featuredCase.image} alt={featuredCase.imageAlt} loading="eager" decoding="async" style={{ objectPosition: featuredCase.imagePosition }} /><span>{featuredCase.category}</span></div>
            <div className="work-reference-featured__content">
              <h3>{featuredCase.title}</h3>
              <p className="work-reference-summary">{featuredCase.summary}</p>
              <div className="work-reference-story">
                <div><span>01</span><strong>The challenge</strong><p>{featuredCase.challenge}</p></div>
                <div><span>02</span><strong>How Digi02 helped</strong><p>{featuredCase.approach}</p></div>
                <div><span>03</span><strong>Operational outcome</strong><p>{featuredCase.outcome}</p></div>
              </div>
              <div className="work-reference-tags" aria-label="Systems used">{featuredCase.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
              <CaseLinks item={featuredCase} />
            </div>
          </article>
        </Container>
      </section>

      <section className="work-reference-named" aria-labelledby="named-case-title">
        <Container>
          <header className="work-reference-section-heading"><p>Selected engagements</p><h2 id="named-case-title">One delivery discipline. Different operating realities.</h2></header>
          <div className="work-reference-named__grid">
            {namedCases.map((item) => <article className="work-reference-named__card" key={item.title}>
              <div className="work-reference-named__media"><img src={item.image} alt={item.imageAlt} loading="eager" decoding="async" style={{ objectPosition: item.imagePosition }} /><span>{item.category}</span></div>
              <div className="work-reference-named__content"><h3>{item.title}</h3><p className="work-reference-summary">{item.summary}</p><dl><div><dt>The challenge</dt><dd>{item.challenge}</dd></div><div><dt>How Digi02 helped</dt><dd>{item.approach}</dd></div><div><dt>Operational outcome</dt><dd>{item.outcome}</dd></div></dl><div className="work-reference-tags">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><CaseLinks item={item} /></div>
            </article>)}
          </div>
        </Container>
      </section>

      <section className="work-reference-applications" aria-labelledby="applications-title">
        <Container>
          <header className="work-reference-section-heading"><p>Operational applications</p><h2 id="applications-title">Built for the operations still taking shape.</h2></header>
          <div className="work-reference-applications__grid">
            {applications.map((item) => <article className="work-reference-applications__card" key={item.title}><div className="work-reference-applications__media" role="img" aria-label={item.imageAlt} style={{ backgroundImage: `url(${item.image})`, backgroundPosition: item.imagePosition }} /><div className="work-reference-applications__content"><p>{item.category}</p><h3>{item.title}</h3><span>{item.summary}</span><a href={item.href}>{item.label} <b aria-hidden="true">→</b></a></div></article>)}
          </div>
        </Container>
      </section>

      <section className="work-reference-method" aria-labelledby="method-title">
        <Container className="work-reference-method__inner"><header><p>How Digi02 delivers</p><h2 id="method-title">Start with the operation. Engineer from there.</h2></header><ol>{deliverySteps.map(([number, title, copy]) => <li key={number}><span>{number}</span><div><h3>{title}</h3><p>{copy}</p></div></li>)}</ol></Container>
      </section>
